...
NODE_BASE_SIZE = 50        # mărimea de bază a nodurilor
EDGE_ALPHA = 0.15          # transparența muchiilor

MAX_GENES_FOR_CORR = 1000
...
def read_expression_matrix_efficient(path: Path, max_genes: int) -> pd.DataFrame:
    """
    Încarcă matricea de expresie eficient:
    - dacă numărul de gene <= max_genes: încarcă normal
    - altfel: parcurge CSV în bucăți pentru a calcula variabilitatea pe gene
      și reîncarcă doar cele mai variabile `max_genes` gene (evitând astfel
      crearea unui DataFrame mare în memorie)
    """
    with open(path, "r") as fh:
        n_lines = sum(1 for _ in fh)
    n_genes = max(0, n_lines - 1)
    if n_genes == 0:
        raise ValueError("Matricea de expresie este goală (0 linii).")

    if n_genes <= max_genes:
        return read_expression_matrix(path)

    chunksize = 500
    best_vars: pd.Series = pd.Series(dtype=float)
    reader = pd.read_csv(path, index_col=0, chunksize=chunksize)
    for chunk in reader:
        vars_chunk = chunk.var(axis=1)
        best_vars = pd.concat([best_vars, vars_chunk]).nlargest(max_genes)

    top_genes = set(best_vars.index)

    parts: list[pd.DataFrame] = []
    reader = pd.read_csv(path, index_col=0, chunksize=chunksize)
    for chunk in reader:
        mask = chunk.index.isin(top_genes)
        if mask.any():
            parts.append(chunk.loc[mask])

    if not parts:
        raise RuntimeError("Nu am reușit să extrag genele selectate din CSV.")
...
if __name__ == "__main__":
    ...
    expr = read_expression_matrix_efficient(EXPR_CSV, MAX_GENES_FOR_CORR)
